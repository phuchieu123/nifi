 ...
 */
@CapabilityDescription("Xác định thông tin xác thực cho các bộ xử lý của Google Cloud Platform. " +
        "Sử dụng thông tin xác thực Mặc định của Ứng dụng (Application Default credentials) mà không cần cấu hình. " +
        "Thông tin xác thực Mặc định của Ứng dụng hỗ trợ biến môi trường (GOOGLE_APPLICATION_CREDENTIALS) trỏ tới " +
        "một tệp thông tin xác thực, cấu hình được tạo bởi lệnh `gcloud auth application-default login`, " +
        "tài khoản dịch vụ của AppEngine/Compute Engine, v.v.")
@Tags({ "gcp", "credentials", "provider" })
@Restricted(
        restrictions = {
                @Restriction(
                        requiredPermission = RequiredPermission.ACCESS_ENVIRONMENT_CREDENTIALS,
                        explanation = "Cấu hình mặc định có thể đọc các biến môi trường và thuộc tính hệ thống để lấy thông tin xác thực"
                )
        }
)
